...
### Code Quality
The project uses Nette Code Checker and Nette Coding Standard (enforced via CI). These are not in composer.json but run in CI:
```bash
# Nette Code Checker (strict types enforcement)
composer create-project nette/code-checker temp/code-checker ^3 --no-progress
php temp/code-checker/code-checker --strict-types --no-progress
